...
type PaperTimeline = {
  fps: number;
  totalFrames: number;
  cues: Array<{ speaker: "A" | "B"; text: string; section: string; startFrame: number; endFrame: number }>;
};
...
export function AnimatedPaper(props: AnimationProps) {
  ...
  const exportJson = () => {
    const blob = new Blob([JSON.stringify({ paperId: props.paperId, ...adjustments }, null, 2)], { type: "application/json" });
    const url = URL.createObjectURL(blob);
    const link = document.createElement("a");
    link.href = url;
    link.download = `${props.paperId}_adjustment.json`;
    link.click();
  ...
  return (
    <div style={{ marginTop: 8 }}>
      ...
      <div style={{ marginTop: 8, display: "flex", justifyContent: "space-between", gap: 12, color: "var(--muted)", fontSize: 12 }}>
        <span>数式・字幕・キャラクターを音声に同期するWeb版</span>
